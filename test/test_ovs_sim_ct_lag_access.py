...
class lagAccessTest( HalonTest ):
    ...
    def config_check(self):
        ...
        s1_getportname_halon, s1_getporttag_halon, s1_getportvlanmode_halon = s1.cmd("/usr/bin/ovs-vsctl get port lag0 name tag vlan_mode").splitlines()
        s1_getportname_openovs, s1_getporttag_openovs, s1_getportvlanmode_openovs = s1.cmd("/opt/openvswitch/bin/ovs-vsctl get port lag0 name tag vlan_mode").splitlines()

        assert s1_getportname_halon == s1_getportname_openovs and s1_getporttag_halon == s1_getporttag_openovs and s1_getportvlanmode_halon == s1_getportvlanmode_openovsi, \
    ...
    def del_and_readd_vlan(self):
        ...
        info("### Ping traffic os expected to fail as the ports get deleted from the OpenSource OVS ###\n\n")
        '''s1.cmd("/usr/bin/ovs-vsctl add-br br0")
        s2.cmd("/usr/bin/ovs-vsctl add-br br0")
        s1.cmd("/usr/bin/ovs-vsctl add-vlan br0 100 admin=up")
        s2.cmd("/usr/bin/ovs-vsctl add-vlan br0 100 admin=up")
        ...
        info("\n################### Adding Global VLAN ###################\n")
        info("\n### Adding VLAN100. Ports 1,lag0(2,3) get reconfigured with tag=100 ###\n")
        ...
        s1.cmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 2")
        time.sleep(2)
        s2.cmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 2")
        time.sleep(2)
        s1.cmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 lag0")
        time.sleep(2)
        s2.cmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 lag0")
        time.sleep(2)
        s1.cmd("/opt/openvswitch/bin/ovs-vsctl add-bond br0 lag0 2 3 vlan_mode=access tag=100")
        time.sleep(2)
        s2.cmd("/opt/openvswitch/bin/ovs-vsctl add-bond br0 lag0 2 3 vlan_mode=access tag=100")
        time.sleep(2)
    ...
    def diff_and_same_tags(self):
        '''
            ...
            3.3 Add port lag0(2,3) with tag 200 on S1 and S2 (access mode)
            3.4 Test Ping - should not work
            3.5 Delete port lag0(2,3) and add port lag0(2,3) with tag 100 on S1 and S2 (access mode)
        ...
        info("\n################### Test Case 3 - With Different Tags ###################\n")
        info("\n### Adding S1->Port 1:tag=100. S2->Port lag0(2,3):tag=200 ###\n")
        info("### Ping traffic is expected to fail as the ports get added with different tags ###\n\n")
        ...
        s1.cmd("/usr/bin/ovs-vsctl add-bond br0 lag0 2 3 vlan_mode=access tag=100 -- set interface 2 user_config:admin=up -- set interface 3 user_config:admin=up")
        time.sleep(2)
        s2.cmd("/usr/bin/ovs-vsctl add-bond br0 lag0 2 3 vlan_mode=access tag=200 -- set interface 2 user_config:admin=up -- set interface 3 user_config:admin=up")
        time.sleep(2)
        s1.cmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 2")
        time.sleep(2)
        s2.cmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 2")
        ...
        s1.cmd("/opt/openvswitch/bin/ovs-vsctl add-bond br0 lag0 2 3 vlan_mode=access tag=100")
        time.sleep(2)
        s2.cmd("/opt/openvswitch/bin/ovs-vsctl add-bond br0 lag0 2 3 vlan_mode=access tag=200")
        time.sleep(2)
        out = h1.cmd("ping -c1 %s" % h2.IP())
        info(out)
        status = parsePing(out)
        if status:
            info("Ping Success\n")
            assert 0, "LAG ports on both the switch have different tags. Successful ping implies test failure"
        else:
            info("Ping Failed\n")
            info("Failure in transmitting packets implies successful test execution")
        ...
        info("\n### Adding Port lag0 in S1, S2 with tag=100. Ports get reconfigured ###\n")
        info("### Ping traffic is expected to pass as the ports get re-added to the OpenSource OVS with same tags ###\n\n")
        ...
        if status:
            ...
            info("Ping Failed\n")
            assert 0, "LAG ports on both the switch have same tags. Packets should get transmitted. Test executuon failure"
