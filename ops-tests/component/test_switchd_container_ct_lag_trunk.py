...
def test_switchd_container_ct_lag_trunk(topology, step):
    ...
    ops1("del-port lag0", shell="vsctl")
    ops2("del-port lag0", shell="vsctl")
    ...
    ops1("add-vlan br0 100 admin=up", shell="vsctl")
    ops2("add-vlan br0 100 admin=up", shell="vsctl")
    ops1("add-port br0 {int} vlan_mode=access tag=100"
         " -- set interface {int} user_config:admin=up".format(int=s1p1),
         shell="vsctl")
    ops2("add-port br0 {int} vlan_mode=access tag=100"
         " -- set interface {int} user_config:admin=up".format(int=s2p1),
         shell="vsctl")
    ops1("add-bond br0 lag0 {int2} {int3} "
         ...
         shell="vsctl")
    sleep(2)
    ops2("add-bond br0 lag0 {int2} {int3} "
         "vlan_mode=trunk trunks=100 -- set interface {int2} "
         "user_config:admin=up -- set interface {int3} "
