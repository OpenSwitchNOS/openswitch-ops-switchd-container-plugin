...
## Sampled Flow
### Objective for sFlow
The test case checks whether sFlow gets successfully configured in the ASIC OVS through OpenSwitch CLI as well as the "hsflowd" daemon that runs on the Switch which is responsible to send the samples and statistics to the collector.
...
```
Also, verify that the configuration is correctly set in "hsflowd" daemon using the following command
```
cat /etc/hsflowd.conf
```
#### Test result criteria for sFlow
##### Test pass criteria for sFlow
The sFlow configuration set through CLI should be seen in ASIC OVS as communication between switch and other devices happens through ASIC OVS. The same configuration should also be seen in "hsflowd.conf" file as it is responsible to send samples and other statistics to the right collector with configured sampling and polling rates. Correct setting of configuration at both the places ensures smooth functionality of sFlow.