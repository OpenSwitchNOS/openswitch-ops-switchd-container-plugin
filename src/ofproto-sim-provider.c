                      ...
                      const struct ofproto_mirror_settings *s)
{
    ...
    if (NULL != s) {
        ...
        if (!mirror) {
            ...
            mirror->aux = aux;
            mirror->out_vlan = -1;
            idx = strnlen(s->name, MAX_MIRROR_NAME_LEN) + 1;
            mirror->name = xzalloc(idx);
            memcpy (mirror->name, s->name, idx);
            if (idx > MAX_MIRROR_NAME_LEN) {
                mirror->name[MAX_MIRROR_NAME_LEN] = '\0'; /* Add NULL termination */
            }
        }
