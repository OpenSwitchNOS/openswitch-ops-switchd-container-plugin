 ...
 int sim_copp_stats_get(const unsigned int hw_asic_id,
    ...
    do {
        ...
        if (nRead < 0) {
            ...
            if (!feof(devFile)) {
                VLOG_ERR("%s: Error reading /proc/net/dev for copp stats '%s'",
                        __FUNCTION__, strerror(errno));
            }
            break;
        }
...
bool isIgnoreRow(char *name, char *ignoreNames[], int listSize)
{
    for (int i = 0; i < listSize; i++) {
        if (!strncmp(name, ignoreNames[i], IF_NAMESIZE)) {
            return true;
        }
    }
    return false;
...
int sim_copp_hw_status_get(const unsigned int hw_asic_id,
    ...
    hw_status->rate = CONTAINER_RATE_VAL;
    hw_status->burst = CONTAINER_RATE_VAL;
