...
class lagAccessTest( OpsVsiTest ):
    ...
    def config_check(self):
        '''
            ...
            1.2 Add port 1 with tag 100 on S1 and S2
                (access mode)
            1.3 Add port lag0(2,3) with tag 100 on S1 and S2
                (access mode)
        ...
        s1_getbrname_ops = s1.ovscmd("/usr/bin/ovs-vsctl get br " \
                                    "br0 name").strip()
        ...
        info("### Switch1: Dual OVS bridge configuration correctly set ###\n")
        ...
        assert s1_getvlanadmin_ops == 'up' and s1_getvlanid_ops == '100' \
               and s1_getvlanname_ops == 'VLAN100', \
               "Switch1: OpenSwitch OVS VLAN configuration mismatch"
        info("### Switch1: OpenSwitch OVS bridge",
             "configuration correctly set ###\n")

        s1_getportname_ops, s1_getporttag_ops, s1_getportvlanmode_ops = \
        s1.ovscmd("/usr/bin/ovs-vsctl get port lag0 name tag " \
               "vlan_mode").splitlines()
        s1_getportname_openovs, s1_getporttag_openovs, \
        ...
        assert s1_getportname_ops == s1_getportname_openovs \
               and s1_getporttag_ops == s1_getporttag_openovs \
        ...
        s1.ovscmd("/opt/openvswitch/bin/ovs-vsctl get " \
        "interface 2 admin_state mac_in_use link_state").splitlines()
    ...
    def add_ports_and_vlan(self):
        '''
            ...
            2.3 Add port lag0(2,3) with tag 100 on S1 and S2 (access mode)
        ...
        info("### Adding ports 1,lag0(2,3) to VLAN100. Port 1:access,",
             "Port lag0(2,3):access ###\n")
        info("### Adding VLAN100 to the VLAN table and adding ports1,",
             "lag(2,3) to bridge: br0\n")
        info("### In access mode and running ping traffic between",
             "the hosts: h1, h2 ###\n")
        ...
        assert status, "Failed to transmit packets when " \
                       "LAG and port are in access mode"
        info("### Ping Success ###\n")
    ...
    def del_and_readd_vlan(self):
        '''
            ...
            3.3 Add port lag0(2,3) with tag 100 on S1 and S2 (access mode)
        ...
        s1.ovscmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 2")
        time.sleep(2)
        s2.ovscmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 2")
        time.sleep(2)
    ...
    def diff_and_same_tags(self):
        '''
            ...
            4.3 Add port lag0(2,3) with tag 200 on S1 and S2 (access mode)
            4.4 Test Ping - should not work
            4.5 Delete port lag0(2,3) and add port lag0(2,3)
                with tag 100 on S1 and S2 (access mode)
        ...
        info("\n########## Test Case 4 - With Different Tags ##########\n")
        info("### Adding S1->Port 1:tag=100.",
             "S2->Port lag0(2,3):tag=200 ###\n")
        info("### Ping traffic is expected to fail as the",
             "ports get added with different tags ###\n\n")
        ...
        s2.ovscmd("/opt/openvswitch/bin/ovs-vsctl del-port br0 lag0")
        time.sleep(2)
        s1.ovscmd("/opt/openvswitch/bin/ovs-vsctl add-bond br0 lag0 " \
               "2 3 vlan_mode=access tag=100")
        time.sleep(2)
        s2.ovscmd("/opt/openvswitch/bin/ovs-vsctl add-bond br0 lag0 " \
               "2 3 vlan_mode=access tag=200")
        time.sleep(2)
        out = h1.cmd("ping -c1 %s" % h2.IP())
        info(out)
        status = parsePing(out)
        assert not status, "LAG ports on both the switch have " \
               "different tags. Successful ping implies test failure"
        info("### Failure in transmitting packets implies",
             "successful test execution ###\n")


        info("### Changing Tags Back to Original ###\n")
        info("### Adding Port lag0 in S1, S2 with tag=100.",
             "Ports get reconfigured ###\n")
        info("### Ping traffic is expected to pass as the ports",
             "get re-added to the SIM OVS with same tags ###\n")
        ...
        info(out)
        status = parsePing(out)
        assert status, "LAG ports on both the switches have same tags. " \
...
class Test_switchd_container_lag_access:
    ...
    def setup_class(cls):
        Test_switchd_container_lag_access.test = lagAccessTest()
    ...
    def teardown_class(cls):
        ...
        # mininet topology
        Test_switchd_container_lag_access.test.net.stop()
