...
class lagAccessTest(OpsVsiTest):
    ...
    def config_check(self):
        '''
            ...
            1.2 Add port 1 with tag 100 on Switch1 and Switch2
                (access mode)
            1.3 Add port lag0(2,3) with tag 100 on Switch1 and Switch2
                (access mode)
        ...
        info("### Switch1: OpenSwitch OVS VLAN",
        ...
        s1_getintfadmin_openovs, s1_getintfmac_openovs, \
            ...
            s1.ovscmd("/opt/openvswitch/bin/ovs-vsctl get interface"
                      " 2 admin_state mac_in_use link_state").splitlines()
        ...
        assert s1_getportname_ops == s1_getportname_openovs \
            and s1_getporttag_ops == s1_getporttag_openovs \
            and s1_getportvlanmode_ops == s1_getportvlanmode_openovs, \
            "Switch1: ASIC OVS bond configuration mismatch after \
                re-adding VLAN"
    ...
    def lag_normal(self):
        '''
            ...
            2.2 Add port 1 with tag 100 on Switch1 and Switch2
                (access mode)
            2.3 Add port lag0(2,3) with tag 100 on Switch1 and Switch2
                (access mode)
    ...
    def lag_missing(self):
        '''
            ...
            3.2 Add port 1 with tag 100 on Switch1 and Switch2
                (access mode)
            3.3 Add port lag0(2,3) with tag 100 on Switch1 and Switch2
                (access mode)
    ...
    def lag_invalid_tags(self):
        '''
            4.1 Add VLAN 100 to global VLAN table on Switch1 and Switch2
            4.2 Add port 1 with tag 100 on Switch1 and Switch2
                (access mode)
            4.3 Add port lag0(2,3) with tag 200 on Switch1 and Switch2
                (access mode)
            4.4 Test Ping - should not work
            4.5 Delete port lag0(2,3) with tag 200 on Switch1 and Switch2
                (access mode)
            4.6 Add port lag0(2,3) with tag 100 on Switch1 and Switch2
                (access mode)
        ...
        info("### Adding ports 1 to VLAN100,lag0(2,3) to VLAN200. ",
             "Port 1:access, Port lag0(2,3):access ###\n")
        info("### Ping traffic is expected to fail as the LAG ports",
             "do not get added in the ASIC OVS ###\n")
        ...
        status = checkPing(out)
        assert not status, "Ping Success even though lag has different tag"
        info("\n### Ping Failed ###\n")

        info("\n\n### Changing tag 200 to 100 in port lag0 ###\n")
        info("### Adding tag 100. Ports lag0(2,3) should get reconfigured",
             "with tag=100 ###\n")
        info("### Ping traffic is expected to pass as the ports",
             "get added with valid tags ###\n")
        s1.ovscmd("/usr/bin/ovs-vsctl del-port lag0")
        s2.ovscmd("/usr/bin/ovs-vsctl del-port lag0")
        time.sleep(2)
        s1.ovscmd("/usr/bin/ovs-vsctl add-bond br0 lag0 2 3 "
        ...
        status = checkPing(out)
        assert status, "Ping Failed even though a valid tag was added. \
                       Ports are expected to get reconfigured"
        info("\n### Ping Success ###\n")
...
class Test_switchd_container_lag_access:
    ...
    def test_switchd_container_lag_invalid_tags(self):
        self.test.lag_invalid_tags()

    def teardown_class(cls):
        # Stop the Docker containers, and ininet topology
